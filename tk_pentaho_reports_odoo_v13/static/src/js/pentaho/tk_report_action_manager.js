...
ActionManager.include({
    ...
    _executeReportAction: function (action, options) {
        ...
        if (action.report_type === 'qweb-html') {
            ...
            return self._tktriggerDownload(action, options, 'pentaho');
        } else {
    ...
    _tkdownloadReport: function (url,action) {
        var self = this;
        framework.blockUI();
        return new Promise(function (resolve, reject) {
            return Model.query({ 
     ...
     * @returns {Promise} resolved when the action has been executed
     */
    _tktriggerDownload: function (action, options, type){
        var self = this;
        // var reportUrls = this._makeReportUrls(action);

        // return this._tkdownloadReport(reportUrls[type]).then(function () {
        return this._tkdownloadReport('/pentaho/download',action).then(function () {
            if (action.close_on_report_download) {
                var closeAction = { type: 'ir.actions.act_window_close' };
                return self.doAction(closeAction, _.pick(options, 'on_close'));
            } else {
    ...
    _makeReportUrls: function (action) {
        var reportUrls = {
            ...
            pentaho: '/report/pentaho/' + action.report_name,
        };
