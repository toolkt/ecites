...
class SignupSuccess(http.Controller):
...
class AuthConfirm(AuthSignupHome):
    ...
    def web_auth_signup(self, *args, **kw):
        ...
        try:
            ...
            message = request.env['mail.message'].with_user(SUPERUSER_ID).search([('res_id', '=', res['partner_id']),
                                                                 ('subject', '=', 'Confirm registration')])
            message.with_user(res['user_id']).set_message_done()
            registration_redirect_url = request.env['ir.config_parameter'].with_user(SUPERUSER_ID).get_param('auth_signup_confirmation.url_singup_thankyou')
    ...
    def singnup_using_generated_link(self, *args, **kw):
        token = kw['token']
        user = request.env['res.users'].with_user(SUPERUSER_ID).with_context(active_test=False).search([
            ('partner_id.signup_token', '=', kw['token'])])
        if user.active:
            pass
        else:
            user.active = True
        success_token_url = '/registration-success/%s' % token
        return werkzeug.utils.redirect(kw.get('redirect') or success_token_url)


    def _singup_with_confirmation(self, *args, **kw):
        old_active_user = request.env['res.users'].with_user(SUPERUSER_ID).search([('login', '=', kw['login'])])
        if old_active_user:
            raise UserExists("A user with this email address is already registered")
        old_not_active_user = request.env['res.users'].with_user(SUPERUSER_ID).with_context(active_test=False).search([
            ('login', '=', kw['login'])])
        if old_not_active_user:
            ...
            new_user = res_users.with_user(SUPERUSER_ID).with_context(
        ...
        new_partner.with_context(link=signup_url,partner=new_partner).message_post_with_template(template.id, composition_mode='comment')
